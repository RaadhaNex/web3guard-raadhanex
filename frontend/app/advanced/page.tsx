  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
      <section className="clean-panel p-6 sm:p-8">
        <p className="section-label">Advanced tools</p>
        ...
        <p className="mt-4 max-w-3xl text-sm leading-7 text-slate-400">
          First users should follow the clean path: Scanner → Results → Report → Pricing. These tools stay here for setup, power users, and internal validation.
        </p>
      </section>

      <section className="mt-6 grid gap-4 lg:grid-cols-3">
        {groups.map((group) => (
          <article key={group.title} className="clean-panel p-6">
            ...
            <div className="mt-5 grid gap-2">
              {group.links.map(([label, href]) => (
                <Link key={href} href={href} className="rounded-xl border border-white/[0.07] bg-white/[0.025] px-4 py-3 text-sm font-bold text-slate-300 transition hover:border-cyan/25 hover:text-white">
                  {label} →
                </Link>
              ))}
            </div>

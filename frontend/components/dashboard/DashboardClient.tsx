...
import { getCurrentUserId, getSessionToken, getSupabaseClient } from "@/lib/supabase";
...
export function DashboardClient() {
  ...
  const [projectName, setProjectName] = useState("My Web3 Launch");
  ...
  async function load() {
    ...
    try {
      ...
      const [statusData, overviewData] = await Promise.all([
        ...
        apiGet<{ overview: Overview }>(`/dashboard/overview?user_id=${encodeURIComponent(currentUserId)}`, { headers }),
      ]);
  ...
  async function createProject() {
    ...
    try {
      ...
      setMessage("Project saved as a real local/Supabase record. No fake scan was created.");
      await load();
    } catch (err) {
  ...
  async function logout() {
    ...
    setMessage("Local honest mode has no real Supabase session to logout.");
  }
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.25em] text-cyan">Phase 10 dashboard</p>
          <h1 className="mt-2 text-4xl font-black">User dashboard + scan history</h1>
          <p className="mt-3 max-w-3xl text-slate-400">This dashboard shows only real saved local/Supabase records. It does not invent fake users, fake scans, fake reports, fake subscription status, or fake audit status.</p>
        </div>
      ...
      <section className="grid gap-4 lg:grid-cols-4">
        <Stat label="Storage" value={dbStatus?.storage_mode_active || "loading"} />
        <Stat label="Supabase" value={dbStatus?.supabase_configured ? "configured" : "not configured"} />
        <Stat label="User" value={email || userId} />
        <Stat label="Auth mode" value={overview?.auth_mode || "loading"} />
      </section>

      {authRequired && (
        <div className="mt-8 rounded-2xl border border-amber-400/30 bg-amber-500/10 p-5 text-amber-100">
          <p className="font-black">Real Supabase auth is configured, but no active session was found.</p>
          <p className="mt-2 text-sm">Login/signup is required. No fake dashboard session was created.</p>
          <Link className="btn-primary mt-4 inline-flex" href="/auth/login">Login</Link>
        </div>
      )}
      {loading && !authRequired && <p className="mt-8 text-slate-400">Loading dashboard...</p>}
      {error && <p className="mt-8 rounded-2xl border border-red-400/30 bg-red-500/10 p-4 text-red-100">{error}</p>}
      {message && <p className="mt-8 rounded-2xl border border-emerald-400/30 bg-emerald-500/10 p-4 text-emerald-100">{message}</p>}

      {!authRequired && overview && (
        <>
          ...
          <section className="mt-8 grid gap-6 lg:grid-cols-[0.85fr_1.15fr]">
            <div className="card p-6">
              ...
              <p className="mt-2 text-sm text-slate-400">Save a real project record before running scans. Scanner pages can save outputs to dashboard using these same APIs.</p>
              <label className="mt-5 block text-sm font-bold text-slate-200">Project name</label>
              <input className="input mt-2" value={projectName} onChange={(event) => setProjectName(event.target.value)} />
              <button className="btn-secondary mt-5" onClick={createProject}>Save project only</button>
            </div>

            <div className="card p-6">
              <h2 className="text-2xl font-black">Real-only note</h2>
              <p className="mt-3 text-slate-300">{overview.real_only_note}</p>
              <div className="mt-5 rounded-2xl border border-white/10 bg-white/[0.03] p-4 text-sm text-slate-400">
                Subscription/payment records are real Phase 8 records only. SecureScore and finding workflows are real Phase 10 saved-scan records only.
              </div>
            </div>
          </section>
  ...
  return (
    <div className="card p-6">
      ...
      <div className="mt-4 grid gap-3">
        {rows.length === 0 ? <p className="text-sm text-slate-500">{empty}</p> : rows.map((row) => (
          <Link key={row.href} href={row.href} className="rounded-2xl border border-white/10 bg-white/[0.03] p-4 transition hover:border-cyan/40 hover:bg-cyan/10">
